...
function validaCampos() {
	...
	if (!Validar_Centro(tipoC)) errores += '(' + (++nErrores) + ') Tipo de centro erroneo.\n';
...
var TelP2;
var EmailP;*/
let campos = [{ nombre: 'tipo', valores: ['primaria', 'secundaria'], defecto: 'no elegido', tipo: 'radio' },/*'tipo',*/'tipoC', 'nombre_centro', 'loc_centro', 'cod_centro', 'Ap11', 'Ap12', 'Nombre1', 'Edad1', 'Ap21', 'Ap22', 'Nombre2', 'Edad2', 'Ap31', 'Ap32', 'Nombre3', 'Edad3', 'Ap41', 'Ap42', 'Nombre4', 'Edad4', 'ApP1', 'ApP2', 'NomP', 'TelP1', 'TelP2', 'EmailP'];

let datosLeidos = {};

function lee() {//alert('entramos');
	campos.forEach(campo => {
		if (campo.tipo) {
			datosLeidos[campo.nombre] = campo.defecto;
			campo.valores.forEach(valor => {
				if (document.getElementById(valor).checked) datosLeidos[campo.nombre] = valor;
			});
		} else datosLeidos[campo] = document.getElementById(campo).value;
	})
	/*
	//tipo="no elegido";
...
var generaPDF = function (imgData) {
	...
	if (document.getElementById('primaria').checked) {
		...
		doc.text(68, 75, tipoC + ' ' + nombre_centro);
...
///funciones para obtener el QR de comprobación se debe utiliar la libreria
/// jquery.qrcode.min.js junto con jquery: https://ajax.googleapis.com/ajax/libs/jquery/1.5.2/jquery.min.js
...
const generaQR = (url, cont, clave) => {
	jQuery('#qr').html('');//se limpia el elemento que contendrá el QR
	jQuery('#qr').qrcode(url);//se llena con el QR
	//let c = $('#qr').children('canvas');//de dicho elemento se toma el canvas con el QR 
	let c = document.getElementById('qr').firstChild.toDataURL();//supongo que es redundante con la de arriba y se comvierta DataUrl
	//se devuelve c:data del QR, la url para imprimirla, y los dos valores de la Queri: contador y clave (que se incluyen en la url)
	return [c, url, cont, clave];
}
