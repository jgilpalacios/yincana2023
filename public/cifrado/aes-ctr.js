...
Aes.Ctr.encrypt = function(plaintext, password, nBits) {
    ...
    //var nonce=1999999999.999 //fijado por mi
    var nonceMs = nonce%1000;
    var nonceSec = Math.floor(nonce/1000);
    var nonceRnd = Math.floor(Math.random()*0xffff);
    //var nonceRnd= 9999; //fijado por mi
...
//FUNCION MODIFICADA PARA QUE DEVUELVA EL TEXTO CIFRADO SIN CODIFICARLO EN BASE 64
/**
 * Encrypt a text using AES encryption in Counter mode of operation.
 *
 * Unicode multi-byte character safe
 ...
 *   var encr = Aes.Ctr.encrypt('big secret', 'pāşšŵōřđ', 256); // encr: 'lwGl66VVwVObKIr6of8HVqJr'
 */
Aes.Ctr.encryptRaw = function(plaintext, password, nBits) {
    var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!(nBits==128 || nBits==192 || nBits==256)) return ''; // standard allows 128/192/256 bit keys
    plaintext = String(plaintext).utf8Encode();
    password = String(password).utf8Encode();
    ...
    // and convert it to a string to go on the front of the ciphertext
    var ctrTxt = '';
    //COMENTADO PARA OBTENER NUMEROS
    //for (var i=0; i<8; i++) ctrTxt += String.fromCharCode(counterBlock[i]);
    for (var i=0; i<8; i++) ctrTxt += 'a'+counterBlock[i];
    ctrTxt += 'c';
    ...
    for (var b=0; b<blockCount; b++) {
        ...
        for (var i=0; i<blockLength; i++) {  // -- xor plaintext with ciphered counter char-by-char --
            cipherChar[i] = cipherCntr[i] ^ plaintext.charCodeAt(b*blockSize+i);
            //COMENTADO PARA NO CONVERTIRLO EN CARACTER
            //cipherChar[i] = String.fromCharCode(cipherChar[i]);
        }
        ciphertxt[b] = cipherChar.join('e');
    }

    // use Array.join() for better performance than repeated string appends
    var ciphertext = ctrTxt + ciphertxt.join('');
    //COMENTADO PARA QUE NO PASE A BASE 64
    //ciphertext = ciphertext.base64Encode();
...
//////////////////------------------------------------------------------------------------------
///////Version modificada por mi para quitar los bloques aleatorios y basados en los milisegundos---------
/**
 * Encrypt a text using AES encryption in Counter mode of operation.
 *
 * Unicode multi-byte character safe
 ...
 *
 * @example
 *   var encr = Aes.Ctr.encryptFijo('big secret', 'pāşšŵōřđ', 256); // encr: 'lwGl66VVwVObKIr6of8HVqJr'
 */
Aes.Ctr.encryptFijo = function(plaintext, password, nBits) {
    var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!(nBits==128 || nBits==192 || nBits==256)) return ''; // standard allows 128/192/256 bit keys
    plaintext = String(plaintext).utf8Encode();
    password = String(password).utf8Encode();
...
//DECODIFICA UN TEXTO CIFRADO QUE NO SE CONVIRTIO A BASE64
/**
 * Decrypt a text encrypted by AES in counter mode of operation
 *
 * @param   {string} ciphertext - Source text to be encrypted.
 ...
 *   var decr = Aes.Ctr.encrypt('lwGl66VVwVObKIr6of8HVqJr', 'pāşšŵōřđ', 256); // decr: 'big secret'
 */
Aes.Ctr.decryptRaw = function(ciphertext, password, nBits) {
    var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!(nBits==128 || nBits==192 || nBits==256)) return ''; // standard allows 128/192/256 bit keys
    //ciphertext = String(ciphertext).base64Decode();
    password = String(password).utf8Encode();
    ...
    // recover nonce from 1st 8 bytes of ciphertext
    var counterBlock = new Array(8);
    //COMENTADO PARA NO CODIFICAR 64
    //var ctrTxt = ciphertext.slice(0, 8);
    var auxCiphertext=ciphertext.split('c');//dividimos lo que interesa es [0] para ctrTxt y [1]
    var ctrTxt =auxCiphertext[0].slice(1);//quitamos la a inicial

    //COMENTADO PARA NO CODIFICAR 64    
    //for (var i=0; i<8; i++) counterBlock[i] = ctrTxt.charCodeAt(i);
    counterBlock=ctrTxt.split('a');

    // generate key schedule
    var keySchedule = Aes.keyExpansion(key);

    //COMENTADA PARA NO CODIFICAR 64
    // separate ciphertext into blocks (skipping past initial 8 bytes)
    //var nBlocks = Math.ceil((ciphertext.length-8) / blockSize);
    auxCiphertext=auxCiphertext[1].split('e');//se convierte en un array de numeros
    var nBlocks =Math.ceil(auxCiphertext.length / blockSize);
    ...
    for (var b=0; b<nBlocks; b++){
	ct[b] = new Array();
	for(var i=0;i<blockSize;i++) ct[b][i]=auxCiphertext[b*blockSize+i];
    }
    ciphertext = ct;  // ciphertext is now array (of block-length strings) de arrays de numeros
